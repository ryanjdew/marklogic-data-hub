  ...
  processResults(results, combinedOptions, flowName, step) {
    ...
    for (let result of results) {
      if (result) {
        ...
        result.context.metadata = self.flowUtils.createMetadata(Object.assign({}, result.context.metadata, combinedOptions.metadata), flowName, step.stepDefinitionName, this.globalContext.jobId);
